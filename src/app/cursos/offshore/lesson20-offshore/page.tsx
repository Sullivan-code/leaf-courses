...
type SectionKey = 'definitions' | 'questions';
...
export default function DPCoursePage() {
    ...
    const definitions = [
        {
            ...
            english: "A software function that maintains thruster output settings when there is a severe system failure, such as total loss of position or heading input to the system. The system continues to estimate the vessel's position based on the last known position, speed, and heading.",
            portuguese: "Uma função de software que mantém as configurações de saída do propulsor quando há uma falha grave do sistema, como a perda total da posição ou da entrada de rumo no sistema. O sistema continua estimando a posição da embarcação com base na última posição conhecida, velocidade e rumo."
        },
        {
            term: "Provision of Redundancy",
            english: "The ability of a DP system to maintain position and heading subsequent to a single point failure. This is achieved by duplicating critical components (thrusters, power supplies, sensors, etc.) so that if one fails, another can take over.",
            portuguese: "A capacidade de um sistema DP de manter a posição e o rumo após uma falha de ponto único. Isso é alcançado duplicando componentes críticos (propulsores, fontes de alimentação, sensores, etc.) para que, se um falhar, outro possa assumir."
        },
        {
            term: "DP Class 1 Vessel",
            english: "A vessel with a DP system that has no redundancy. If a single component in the DP system fails, the vessel may lose position and/or heading. No single failure is required to be withstood.",
            portuguese: "Uma embarcação com um sistema DP que não possui redundância. Se um único componente no sistema DP falhar, a embarcação pode perder posição e/ou rumo. Nenhuma falha única é necessária para ser suportada."
        },
        {
            term: "Consequence Analysis (online)",
            english: "A software function that verifies continued thruster and power redundancy for DP Class 2 and 3 vessels. It performs calculations to verify that sufficient thrust is available to maintain position even after a single point failure.",
            portuguese: "Uma função de software que verifica a continuidade do propulsor e da redundância de energia para embarcações DP Classe 2 e 3. Ela realiza cálculos para verificar se há empuxo suficiente disponível para manter a posição mesmo após uma falha de ponto único."
        },
        {
            term: "DGNSS / DGPS",
            english: "Differential Global Navigation Satellite System / Differential Global Positioning System. A position reference system that provides the DP system with a global position (latitude, longitude, or UTM coordinates). It uses ground-based reference stations to correct GPS signals, improving accuracy.",
            portuguese: "Sistema Global de Navegação por Satélite Diferencial / Sistema de Posicionamento Global Diferencial. Um sistema de referência de posição que fornece ao sistema DP uma posição global (latitude, longitude ou coordenadas UTM). Ele usa estações de referência terrestres para corrigir sinais GPS, melhorando a precisão."
        },
        {
            term: "Severe System Failure / Total Loss",
            english: "A catastrophic failure in the DP system, such as total loss of position or heading input. When this occurs, the system enters Dead Reckoning mode, where the software maintains thruster output settings based on the mathematical model until the operator takes action.",
            portuguese: "Uma falha catastrófica no sistema DP, como a perda total da posição ou da entrada de rumo. Quando isso ocorre, o sistema entra no modo Dead Reckoning, onde o software mantém as configurações de saída do propulsor com base no modelo matemático até que o operador tome uma ação."
        },
        {
            term: "FMEA (Failure Modes and Effects Analysis)",
            english: "A document which highlights failure modes of the DP system and their effects on operation. It is a systematic method for identifying potential failures in a system and assessing their impact. For DP vessels, it is used to design redundant systems and to train operators.",
            portuguese: "Um documento que destaca os modos de falha do sistema DP e seus efeitos na operação. É um método sistemático para identificar falhas potenciais em um sistema e avaliar seu impacto. Para embarcações DP, é usado para projetar sistemas redundantes e treinar operadores."
        },
        {
            term: "Capability Diagram",
            english: "A tool for estimating the position-keeping capability of the ship under various environmental conditions (wind, waves, current). It shows the maximum environmental forces the vessel can withstand while maintaining position.",
            portuguese: "Uma ferramenta para estimar a capacidade de manutenção de posição do navio sob várias condições ambientais (vento, ondas, corrente). Mostra as forças ambientais máximas que a embarcação pode suportar enquanto mantém a posição."
        },
        {
            term: "Power Management System (PMS)",
            english: "A system that manages the electrical power on a DP vessel. Its main function is to prevent blackouts by automatically starting and stopping generators, managing load, and shedding non-essential loads when necessary.",
            portuguese: "Um sistema que gerencia a energia elétrica em uma embarcação DP. Sua principal função é prevenir apagões iniciando e parando geradores automaticamente, gerenciando carga e descartando cargas não essenciais quando necessário."
        },
        {
            term: "USBL (Ultra Short Baseline)",
            english: "An acoustic positioning system that determines position by measuring the range and bearing of a single transponder beacon from a vessel-mounted transducer array. The transducer array has a known geometry (short baseline) and measures the angle and distance to the beacon.",
            portuguese: "Um sistema de posicionamento acústico que determina a posição medindo o alcance e o rumo de um único transponder a partir de um conjunto de transdutores montado na embarcação. O conjunto de transdutores tem uma geometria conhecida (linha de base curta) e mede o ângulo e a distância até o beacon."
        },
        {
            term: "LBL (Long Baseline)",
            english: "An acoustic positioning system that uses a single transducer on the vessel and a calibrated array of transponders located on the sea floor. The vessel measures distances to multiple transponders to determine its position relative to the array.",
            portuguese: "Um sistema de posicionamento acústico que utiliza um único transdutor na embarcação e um conjunto calibrado de transponders localizados no fundo do mar. A embarcação mede distâncias para múltiplos transponders para determinar sua posição em relação ao conjunto."
        },
        {
            term: "HPR (Hydroacoustic Position Reference)",
            english: "An underwater acoustic positioning system that uses sound waves to determine the position of a vessel relative to a subsea beacon or transponder. Performance can be limited by acoustic conditions such as noise from thrusters, temperature layers, and bubbles.",
            portuguese: "Um sistema de posicionamento acústico subaquático que usa ondas sonoras para determinar a posição de uma embarcação em relação a um beacon ou transponder subaquático. O desempenho pode ser limitado por condições acústicas, como ruído de propulsores, camadas de temperatura e bolhas."
        },
        {
            term: "Auto Position Mode",
            english: "A DP system mode where the system automatically controls the vessel's position and heading to maintain them as closely as possible to the set-point values. The system uses thrusters to counteract environmental forces and maintain the desired position.",
            portuguese: "Um modo do sistema DP onde o sistema controla automaticamente a posição e o rumo da embarcação para mantê-los o mais próximo possível dos valores de set-point. O sistema usa propulsores para neutralizar forças ambientais e manter a posição desejada."
        },
        {
            term: "Control Gain",
            english: "A parameter in the DP control system that determines how aggressively the system responds to position errors. Increasing the gain makes the system respond faster but can cause instability. It is often increased when encountering higher wind or current conditions.",
            portuguese: "Um parâmetro no sistema de controle DP que determina quão agressivamente o sistema responde a erros de posição. Aumentar o ganho faz o sistema responder mais rapidamente, mas pode causar instabilidade. Geralmente é aumentado ao encontrar condições de vento ou corrente mais fortes."
        },
        {
            term: "Sway, Surge, Yaw",
            english: "Three of the six degrees of freedom of a vessel. Sway is the lateral (side-to-side) movement. Surge is the longitudinal (forward-backward) movement. Yaw is the rotation around the vertical axis (heading). These three are controlled by the DP system.",
            portuguese: "Três dos seis graus de liberdade de uma embarcação. Sway é o movimento lateral (de um lado para o outro). Surge é o movimento longitudinal (para frente e para trás). Yaw é a rotação em torno do eixo vertical (rumo). Esses três são controlados pelo sistema DP."
        }
    ];

    // ============================================
    // DATA: All Questions from the Document
    // ============================================
    ...
    const questions: Question[] = [
        ...
        {
            id: 1,
            question: "Dynamic positioning can be defined as a system which?",
        ...
        // Q32 - skipping because document is incomplete for this question
        // Q33
        {
            id: 33,
        ...
        // Q49 - duplicate of 48 essentially
        {
            id: 49,
        ...
        // Q51
        {
            ...
            isCorrect: (ans) => ans === "C" || ans === "D"
        },
        // Extra: Follow-target mode
        {
            id: "52",
        ...
        // Extra: Most important component
        {
            id: "54",
        ...
        // Extra: Priority for thrust allocation
        {
            id: "58",
        ...
        // Extra: Auto-DP control statement
        {
            id: "60",
        ...
        // Extra: Auto-heading in Joystick
        {
            id: "61",
        ...
        // Extra: Degree monitored not controlled
        {
            id: "62",
        ...
        // Extra: Heading control in Auto-DP
        {
            id: "63",
        ...
        // Extra: Degree not dependent on PRS
        {
            id: "67",
        ...
        // Extra: Disadvantage of DP (duplicate of Q27)
        {
            id: "68",
        ...
        // Extra: Shallow water NOT significant
        {
            id: "70",
            ...
            <div className="max-w-6xl mx-auto bg-[#f0f8ff] bg-opacity-95 rounded-[40px] p-10 shadow-lg">
                ...
                <div className="text-center mb-10">
                    <h1 className="text-5xl font-bold text-[#0c4a6e] mb-4">
                        ⚓ Dynamic Positioning (DP) - Complete Course
                    </h1>
                ...
                <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
                    <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
                        ...
                        >
                            {openSections.definitions ? 'Hide' : 'Show'}
                        </button>
                    </div>
                ...
                <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
                    <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
                        ...
                        <button
                            ...
                            className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-6 py-2 text-sm transition-all duration-300"
                        >
                            {openSections.questions ? 'Hide' : 'Show All'}
                        </button>
                    </div>

                    {openSections.questions && (
                        <div className="p-8" style={{ animation: 'fadeIn 0.3s ease-out' }}>
                            <SpeakSentence text="Click on each question to hear it spoken. Click on the answer to hear the correct response." className="text-md text-gray-600 mb-4 italic">
                                🎧 Click on each question to hear it spoken. Click on the answer to hear the correct response.
                            </SpeakSentence>

                            <div className="space-y-6">
                                {questions.map((q, idx) => (
                                    <div key={idx} className="bg-gray-50 rounded-xl p-5 border border-gray-200 hover:border-blue-300 transition-colors">
                                        <div className="flex items-start gap-2">
                                            <span className="text-blue-600 font-bold text-sm min-w-[30px]">Q{q.id}.</span>
                                            <div className="flex-1">
                                                {/* Question in English with audio */}
                                                        ...
                                                        {q.options.map((opt, oi) => (
                                                            <div key={oi} className="flex items-start gap-2 text-sm">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    {opt.textPt && <span className="text-gray-400 text-xs ml-2">({opt.textPt})</span>}
                                                                </div>
                                                            </div>
                                                        ))}
                                                    </div>
                                                ...
                                                <div className="mt-3 pt-3 border-t border-gray-200">
                                                    <div className="flex items-start gap-2">
                                                        ...
                                                        <div>
                                                            <SpeakSentence text={q.answer} className="text-green-700 font-medium cursor-pointer hover:text-green-900">
                                                                {q.answer}
                                                            </SpeakSentence>
                                                            {q.answerPt && (
