...
// ============================================
// SPEECH SYSTEM (mesmo das lições anteriores)
// ============================================
...
export default function Lesson21() {
  ...
  const toggleTranslation = (section: keyof typeof showTranslations) => {
    setShowTranslations({ ...showTranslations, [section]: !showTranslations[section] });
  };
  ...
  // --- IMAGE URL (mesma imagem para todos os espaços) ---
  const imageUrl =
  ...
  const vocabulary = [
    { term: "Blackout", definition: "Complete loss of electrical power on the vessel.", pt: "Apagão: perda total de energia elétrica na embarcação." },
    { term: "Drive-off", definition: "Uncontrolled movement of the vessel away from its intended position.", pt: "Deriva incontrolável: movimento não controlado da embarcação para fora de sua posição." },
    { term: "Thruster failure", definition: "Loss of thrust from one or more thrusters.", pt: "Falha de propulsor: perda de empuxo de um ou mais propulsores." },
    { term: "Power Management System (PMS)", definition: "System that manages power generation and distribution to prevent blackouts.", pt: "Sistema de Gerenciamento de Energia (PMS): gerencia a geração e distribuição de energia para evitar apagões." },
    { term: "Emergency generator", definition: "Backup generator that starts automatically during a power failure.", pt: "Gerador de emergência: gerador reserva que liga automaticamente durante uma falha de energia." },
    { term: "Manual override", definition: "Manual control that bypasses the automatic system.", pt: "Substituição manual: controle manual que contorna o sistema automático." },
    { term: "Alarm", definition: "Audible and visual warning to indicate a problem.", pt: "Alarme: aviso sonoro e visual para indicar um problema." },
    { term: "Warning", definition: "Indication of a potential issue that may require attention.", pt: "Advertência: indicação de um problema potencial que pode exigir atenção." },
    { term: "Bridge", definition: "The command center of the vessel where navigation and DP are controlled.", pt: "Ponte: centro de comando da embarcação onde a navegação e o DP são controlados." },
    { term: "Engine room", definition: "The compartment containing the main engines and generators.", pt: "Sala de máquinas: compartimento que contém os motores principais e geradores." },
    { term: "Thruster room", definition: "The space housing the thrusters and their control equipment.", pt: "Sala de propulsores: espaço que abriga os propulsores e seus equipamentos de controle." },
    { term: "DP console", definition: "The control panel from which the DPO operates the DP system.", pt: "Console DP: painel de controle a partir do qual o DPO opera o sistema DP." },
    { term: "Redundancy", definition: "Duplication of critical components to ensure continued operation after a failure.", pt: "Redundância: duplicação de componentes críticos para garantir operação contínua após uma falha." },
    { term: "FMEA", definition: "Failure Modes and Effects Analysis – a document detailing potential failures and their consequences.", pt: "FMEA: Análise de Modos e Efeitos de Falha – documento que detalha falhas potenciais e suas consequências." },
    { term: "Consequence Analysis", definition: "Software function that verifies if sufficient thrust is available after a single point failure.", pt: "Análise de Consequências: função de software que verifica se há empuxo suficiente após uma falha de ponto único." },
  ];

  // --- READING: "A DP Emergency" ---
  const reading = {
  ...
  // --- CONVERSATION: "Dealing with a Thruster Failure" ---
  const conversation = [
    { speaker: "DPO:", line: "Captain, we have a thruster failure on thruster 2. The vessel is drifting.", pt: "Capitão, temos uma falha no propulsor 2. A embarcação está derivando." },
    { speaker: "Captain:", line: "Understood. Start the emergency generator and switch to manual override if necessary.", pt: "Entendido. Ligue o gerador de emergência e mude para substituição manual se necessário." },
    { speaker: "DPO:", line: "I am already in manual override. I'm using thrusters 1 and 3 to hold position.", pt: "Já estou em substituição manual. Estou usando os propulsores 1 e 3 para segurar a posição." },
    { speaker: "Chief Engineer:", line: "I'm in the engine room. The thruster motor overheated. I'm resetting the breaker.", pt: "Estou na sala de máquinas. O motor do propulsor superaqueceu. Estou resetando o disjuntor." },
    { speaker: "DPO:", line: "Copy. We have a 5-degree heading error. Please hurry.", pt: "Entendido. Temos um erro de rumo de 5 graus. Por favor, apresse-se." },
    { speaker: "Chief Engineer:", line: "Breaker reset. Trying to re-engage the thruster now.", pt: "Disjuntor resetado. Tentando reengajar o propulsor agora." },
    { speaker: "DPO:", line: "Thruster 2 is back online. Position recovering. We are stable.", pt: "Propulsor 2 está de volta. Posição recuperando. Estamos estáveis." },
    { speaker: "Captain:", line: "Good work, everyone. Log the incident and conduct a full system check.", pt: "Bom trabalho, todos. Registrem o incidente e façam uma verificação completa do sistema." }
  ];

  // --- GRAMMAR: Conditionals and Modal Verbs ---
  const grammarPoints = [
    {
      title: "Conditional Sentences (If...then)",
      examples: [
        { en: "If a thruster fails, the system will compensate with the remaining thrusters.", pt: "Se um propulsor falhar, o sistema compensará com os propulsores restantes." },
        { en: "If we lose power, we must start the emergency generator immediately.", pt: "Se perdermos energia, devemos ligar o gerador de emergência imediatamente." },
        { en: "If the DPO switches to manual override, he can control the thrusters directly.", pt: "Se o DPO mudar para substituição manual, ele pode controlar os propulsores diretamente." },
      ]
    },
    {
      title: "Modal Verbs (Must, Should, Have to)",
      examples: [
        { en: "You must inform the captain of any system failure.", pt: "Você deve informar o capitão sobre qualquer falha do sistema." },
        { en: "You should reduce the load on the generators during a blackout.", pt: "Você deve reduzir a carga nos geradores durante um apagão." },
        { en: "The crew has to follow the emergency procedures.", pt: "A tripulação tem que seguir os procedimentos de emergência." },
      ]
    }
  ];
  ...
  const practiceExercises = [
    {
      ...
      items: [
        ...
        { sentence: "The __________ system manages power to prevent blackouts.", answer: "Power Management" },
        { sentence: "In case of failure, the DPO can use __________ to bypass automatic control.", answer: "manual override" },
      ]
    },
    {
      type: "translate",
  ...
  const speakingPractice = [
    { role: "DPO", scenario: "You are on the DP console. An alarm indicates a loss of heading reference. What do you do?", pt: "Você está no console DP. Um alarme indica perda de referência de rumo. O que você faz?" },
    { role: "Chief Engineer", scenario: "You receive a call from the DPO about a thruster failure. You go to the engine room. Describe your actions.", pt: "Você recebe uma chamada do DPO sobre uma falha de propulsor. Você vai à sala de máquinas. Descreva suas ações." },
    { role: "Captain", scenario: "You are informed of a drive-off situation. Give orders to the DPO and the crew.", pt: "Você é informado sobre uma situação de deriva. Dê ordens ao DPO e à tripulação." },
    { role: "ROV Pilot", scenario: "You are about to deploy the ROV when the DPO tells you to stand by due to a power issue. How do you respond?", pt: "Você está prestes a lançar o ROV quando o DPO lhe diz para aguardar devido a um problema de energia. Como você responde?" },
  ];

  // --- EXAM QUESTIONS (15 new questions about failures) ---
  const questions = [
    {
      id: 1,
    ...
    },
    {
      ...
      options: [
        ...
        { letter: "B", text: "Inform the Captain and Chief Engineer, and consider manual override.", textPt: "Informar o Capitão e o Chefe de Máquinas, e considerar a substituição manual." },
    ...
    >
      <div className="max-w-6xl mx-auto bg-[#f0f8ff] bg-opacity-95 rounded-[40px] p-10 shadow-lg">
        ...
        <div className="text-center mb-12">
          <h1 className="text-5xl font-bold text-[#0c4a6e] mb-4">⚓ Lesson 21 — Beyond Failure: Troubleshooting DP Emergencies</h1>
          <SpeakSentence text="Learn how to handle failures and emergencies in Dynamic Positioning operations." className="text-xl text-gray-700 max-w-3xl mx-auto">
        ...
        <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
            <div className="flex items-center">
              <h2 className="text-2xl font-bold">🔹 Key Vocabulary – Failure & Response</h2>
              <PencilIcon onClick={() => openNoteModal("Vocabulary")} />
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => toggleTranslation("vocabulary")}
              ...
              >
                {showTranslations.vocabulary ? "Ocultar traduções" : "Mostrar traduções"}
              </button>
              <button
                onClick={() => toggleSection("vocabulary")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {openSections.vocabulary ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
        ...
        <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
            <div className="flex items-center">
              <h2 className="text-2xl font-bold">🔹 Reading – A DP Emergency</h2>
              <PencilIcon onClick={() => openNoteModal("Reading")} />
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => toggleTranslation("reading")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {showTranslations.reading ? "Ocultar tradução" : "Mostrar tradução"}
              </button>
              <button
                onClick={() => toggleSection("reading")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {openSections.reading ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
        ...
        <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
            <div className="flex items-center">
              <h2 className="text-2xl font-bold">🔹 Conversation – Dealing with a Thruster Failure</h2>
              <PencilIcon onClick={() => openNoteModal("Conversation")} />
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => toggleTranslation("conversation")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {showTranslations.conversation ? "Ocultar tradução" : "Mostrar tradução"}
              </button>
              <button
                onClick={() => toggleSection("conversation")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {openSections.conversation ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
        ...
        <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
            <div className="flex items-center">
              <h2 className="text-2xl font-bold">🔹 Grammar – Conditionals & Modal Verbs</h2>
              <PencilIcon onClick={() => openNoteModal("Grammar")} />
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => toggleTranslation("grammar")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {showTranslations.grammar ? "Ocultar tradução" : "Mostrar tradução"}
              </button>
              <button
                onClick={() => toggleSection("grammar")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {openSections.grammar ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
        ...
        <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
            <div className="flex items-center">
              ...
              <PencilIcon onClick={() => openNoteModal("Practice")} />
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => toggleTranslation("practice")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {showTranslations.practice ? "Ocultar tradução" : "Mostrar tradução"}
              </button>
              <button
                onClick={() => toggleSection("practice")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {openSections.practice ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
                        ...
                        {exercise.type === "fill" ? (
                          <>
                            <p className="text-gray-800">
                              {item.sentence.replace("__________", "________")}
                              <span className="text-green-600 font-bold ml-2">→ {item.answer}</span>
                            </p>
                            {showTranslations.practice && (
                              <p className="text-sm text-gray-500">🇧🇷 {item.sentence.replace("__________", "________")} → {item.answer}</p>
                            )}
                          </>
                        ) : (
                          <>
        ...
        <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
            <div className="flex items-center">
              <h2 className="text-2xl font-bold">🔹 Speaking Practice – Role Play</h2>
              <PencilIcon onClick={() => openNoteModal("Speaking")} />
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => toggleTranslation("speaking")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {showTranslations.speaking ? "Ocultar tradução" : "Mostrar tradução"}
              </button>
              <button
                onClick={() => toggleSection("speaking")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {openSections.speaking ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
          {openSections.speaking && (
            <div className="p-8" style={{ animation: "fadeIn 0.3s ease-out" }}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
            <div className="flex items-center">
              <h2 className="text-2xl font-bold">🔹 Exam Questions – Failures & Emergencies</h2>
              <PencilIcon onClick={() => openNoteModal("Questions")} />
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => toggleTranslation("questions")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {showTranslations.questions ? "Ocultar traduções" : "Mostrar traduções"}
              </button>
              <button
                onClick={() => toggleSection("questions")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {openSections.questions ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
          {openSections.questions && (
            <div className="p-8" style={{ animation: "fadeIn 0.3s ease-out" }}>
              <p className="text-sm text-gray-600 mb-4">🎧 Click on each question to hear it. Use the options or type your answer.</p>
              <div className="space-y-6">
                {questions.map((q, idx) => (
                  <QuestionItem key={idx} q={q} index={idx} showTranslation={showTranslations.questions} />
                ))}
              </div>
            </div>
          )}
        ...
        <div className="bg-white border-2 border-blue-200 rounded-[30px] shadow-lg mb-10 overflow-hidden">
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white py-4 px-8 flex justify-between items-center">
            <div className="flex items-center">
              <h2 className="text-2xl font-bold">⭐ Final Review – Key Takeaways</h2>
              <PencilIcon onClick={() => openNoteModal("Review")} />
            </div>
            <div className="flex gap-2">
              <button
                ...
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {showTranslations.review ? "Ocultar tradução" : "Mostrar tradução"}
              </button>
              <button
                onClick={() => toggleSection("review")}
                className="inline-block rounded-full bg-white/20 hover:bg-white/30 text-white px-4 py-1.5 text-sm transition-all duration-300"
              >
                {openSections.review ? "Ocultar" : "Mostrar"}
              </button>
            </div>
          </div>
          {openSections.review && (
...
// ============================================
// Subcomponent: QuestionItem (similar to lesson 20)
// ============================================

function QuestionItem({ q, index, showTranslation }: { q: any; index: number; showTranslation: boolean }) {
